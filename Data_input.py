...
direct_target = 'diseasename/omim.csv'
neighbor_num = 2
...
def target_mol(filepath , filename, tar = 'all', direct= 0): #根据指定的靶点找出相对应的成分，all为默认的全量数据,direct=1表示直接使用靶点
...
def targets_mol_herb(filepath, filename):#生成目标靶点对应的成分和中药的矩阵
    target_molecules = target_mol(filepath ,filename, 0,0)  # 靶点成分对应关系
